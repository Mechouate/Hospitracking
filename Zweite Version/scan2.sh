...
#max2=$(sudo mysql --user=root --password=naima raum -e"select Maximum,Max_date from objekt_information where Maximum=(select min(Maximum) from objekt_information where objekt_mac_adr='${mac}');"|sed -n 2p|awk '{print $1}')
#max_d=$(date '+%Y-%m-%d %H:%M:%S')
#md=$(sudo mysql --user=root --password=naima raum -e"select Maximum,Max_date from objekt_information where Maximum=(select min(Maximum) from objekt_information where objekt_mac_adr='${mac}');"|sed -n 2p|awk '{print $2 " " $3}')
rm $mac.5.csv
...
INSERT INTO $TABLE (objekt_mac_adr, RSSI_Max1, RSSI_Max2, RSSI_Max3, Mittelwert,Maximum, objekt_ort ) VALUES ("$mac","$v1", "$v2","$v3","$mw","$max1"  ,"$host");
EOF